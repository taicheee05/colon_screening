...
if uploaded_file is not None:
    ...
    # コストの設定をストリームリットで入力
    c_ma_f1_dukesA = st.number_input('Cost for Duke A (c_ma_f1_dukesA)', value=1786482)
    c_ma_f1_dukesB = st.number_input('Cost for Duke B (c_ma_f1_dukesB)', value=2056922)
    c_ma_f1_dukesC = st.number_input('Cost for Duke C (c_ma_f1_dukesC)', value=2637803)
    c_ma_f1_dukesD = st.number_input('Cost for Duke D (c_ma_f1_dukesD)', value=3179764)
    c_ma_f2_dukesA = st.number_input('Cost for follow-up Duke A (c_ma_f2_dukesA)', value=34610)
    c_ma_f2_dukesB = st.number_input('Cost for follow-up Duke B (c_ma_f2_dukesB)', value=34610)
    c_ma_f2_dukesC = st.number_input('Cost for follow-up Duke C (c_ma_f2_dukesC)', value=43758)
    c_ma_f2_dukesD = st.number_input('Cost for follow-up Duke D (c_ma_f2_dukesD)', value=2476258)

    # ユーティリティの設定をストリームリットで入力
    u_dukeA = st.number_input('Utility for Duke A (u_dukeA)', value=0.879)
    u_dukeB = st.number_input('Utility for Duke B (u_dukeB)', value=0.879)
    u_dukeC = st.number_input('Utility for Duke C (u_dukeC)', value=0.867)
    u_dukeD = st.number_input('Utility for Duke D (u_dukeD)', value=0.844)
    u_normal = st.number_input('Utility for Normal (u_normal)', value=1.0)
